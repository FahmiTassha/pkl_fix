 ...
 {{-- Sidebar --}}
        ...
        <div class="flex-1 p-8">
            <div class="flex justify-between items-center mb-6"> 
                <div>
                    <h1 class="text-4xl font-medium text-black flex items-center">
                     <img src="{{ asset('icon/panahkiri.png') }}" alt="panah" class="w-8 h-8 mr-2">{{ $travel->nama }}</h1>
                    <p class="text-sm pt-3 text-gray-500">
                        Admin / Paket Travel / 
                        <span class="text-black">{{ $travel->nama }}</span>
                    </p>
                </div>
            </div>
            ...
            <form action="{{ route('admin.travel.update', $travel->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700">Nama Paket Travel</label>
                ...
                <div class="mb-4">
                    <label class="block text-gray-700">Orang</label>
                    <input type="number" name="orang" value="{{ old('tempat', $travel->orang) }}"
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700">Harga / Hari</label>
                    <input type="number" name="harga" value="{{ old('harga', $travel->harga) }}"
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700">Deskripsi Paket</label>
                    <input type="text" name="deskripsi" value="{{ old('deskripsi', $travel->deskripsi) }}"
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700">Destinasi</label>
                    <input type="text" name="destinasi" value="{{ old('destinasi', $travel->destinasi) }}"
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700">Include</label>
                    <input type="text" name="insclude" value="{{ old('include', $travel->include) }}"
                        class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    @if($travel->gambar)
                        <img src="{{ asset('tour/' . $travel->gambar) }}" alt="{{ $travel->nama }}"
