...
<head>
    ...
    <style>
        ...
        #mobilTable th:first-child,
        #mobilTable td:first-child {
        ...
        #mobilTable th:not(:first-child),
        #mobilTable td:not(:first-child) {
            border-left: 1px solid #ccc !important;
            border-right: 1px solid #ccc !important;
        }
        /* Kolom terakhir (Aksi) → border kanan transparan */
        #mobilTable th:last-child,
...
<body class="bg-gray-100">
    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 p-8">
            ...
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h1 class="text-4xl font-medium text-black">Paket Travel</h1>
                    <p class="text-sm pt-3 text-gray-500">Admin / <span class="text-black">Paket Travel</span></p>
                </div>
            </div>
            ...
            {{-- Tabel Travel --}}
            <div id="mobilTable" class="bg-white mt-8 px-5 py-4 rounded-md">
                <table class="w-full text-left border-collapse">
                    <thead class="bg-gray-100 text-[#6C757D]">
                        <tr>
                            ...
                            <th class="px-4 py-2 border font-normal">Orang</th>
                            <th class="px-4 py-2 border font-normal w-32">Harga / Hari</th>
                            <th class="px-4 py-2 border font-normal w-48">Deskripsi</th>
                            <th class="px-4 py-2 border font-normal w-48">Destinasi</th>
                            <th class="px-4 py-2 border font-normal w-48">Include</th>
                    ...
                    <tbody id="mobilTableBody">
                        @foreach ($travels as $i => $travel)
                            <tr>
                                ...
                                <td class="px-4 py-2 border">{{ $travel->nama }}</td>
                                ...
                                <td class="px-4 py-2 border">{{ $travel->orang }}</td>
                                <td class="px-4 py-2 border">Rp {{ number_format($travel->harga, 0, ',', '.') }}</td>
                                <td class="px-4 py-2 border max-w-[200px] truncate">{{ $travel->deskripsi }}</td>
                                <td class="px-4 py-2 border max-w-[200px] truncate">{{ $travel->destinasi }}</td>
                                <td class="px-4 py-2 border max-w-[200px] truncate">{{ $travel->include }}</td>
                                <td class="px-4 py-2 border text-center space-x-2">
                                    <a href="{{ route('admin.travel.show', $travel->id) }}"
                                    ...
                                    <form action="{{ route('admin.travel.destroy', $travel->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="px-2 py-1 border rounded hover:bg-red-100 text-red-600"
                                            onclick="return confirm('Yakin hapus paket travel ini?')">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const rows = Array.from(document.querySelectorAll('#mobilTableBody tr'));
